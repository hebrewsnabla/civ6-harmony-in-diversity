...
-- SIZE_NODE_X and SIZE_NODE_X_HALF depends on Unit Promotion Class
function get_SIZE_NODE_X(row)
	if (row == nil) or (row.PromotionClass ~= 'PROMOTION_CLASS_MONK') then
		return SIZE_NODE_X;
	else
		return 220;
	end
end
function get_SIZE_NODE_X_HALF(row)
	if (row == nil) or (row.PromotionClass ~= 'PROMOTION_CLASS_MONK') then
		return SIZE_NODE_X_HALF;
	else
		return 135;
	end
end
...
function OnPromoteUnitPopup()
		...
		for row in GameInfo.UnitPromotionPrereqs() do
			...
			if row.PrereqUnitPromotion == i.UnitPromotionType then
				...
				if (currentColumn > previousColumn or currentColumn < previousColumn) and  (currentRow > previousRow or currentRow < previousRow) then
					...
					local PrereqX = currentColumn*get_SIZE_NODE_X(i) - get_SIZE_NODE_X_HALF(i);
					local MyX = previousColumn*get_SIZE_NODE_X(i) - get_SIZE_NODE_X_HALF(i);
					...
					if(currentColumn > previousColumn or currentColumn < previousColumn) then
						local startX:number= (math.min(previousColumn,currentColumn))*get_SIZE_NODE_X(i);
						local endX :number = (math.max(previousColumn,currentColumn)-1)*get_SIZE_NODE_X(i);
						...
						local XVal:number = currentColumn*get_SIZE_NODE_X(i) - get_SIZE_NODE_X_HALF(i);
	...
	for row in GameInfo.UnitPromotions() do
		...
		if row.PromotionClass == promotionClass then
			...
			promotionInstance.PromotionSelection:SetOffsetVal((row.Column-1)*get_SIZE_NODE_X(row), (row.Level-1)*SIZE_NODE_Y);
