...
-------------------------------------------------------------------------------- hd begin
function GetBeliefsSortIndex(row)
	return GameInfo.BeliefsSortIndex[row.BeliefType].SortIndex;
end
function PopulateAvailableBeliefs(beliefType:string) -- 选信条，nil for 其他所有, FOLLOWER for 追随
	local Beliefs : table = {};
	-- print("running hd religion screen.")
	...
	for row in GameInfo.Beliefs() do
		...
		if (not bBeliefTypeAlreadySelected and 
			...
			-- not m_pGameReligion:IsInSomeReligion(row.Index) and -- remove this to see beliefs in other religion.
			not m_pGameReligion:IsTooManyForReligion(row.Index, m_PlayerReligionType) and
			((beliefType ~= nil and row.BeliefClassType == beliefType) or -- condition choose Follower
			 (beliefType == nil and row.BeliefClassType ~= "BELIEF_CLASS_PANTHEON"))) then -- condition choose Others
			table.insert(Beliefs, row);
		end
	end

	table.sort(Beliefs, function(a, b)
		return GetBeliefsSortIndex(a) < GetBeliefsSortIndex(b);
		end );
	...
	for _, row in ipairs(Beliefs) do
		...
		beliefInst.BeliefDescription:LocalizeAndSetText(row.Description); -- add label and description.
		SetBeliefIcon(beliefInst.BeliefIcon, row.BeliefType, SIZE_BELIEF_ICON_LARGE);
		SetBeliefSlotDisabled(beliefInst, false);
		
		if not m_pGameReligion:IsInSomeReligion(row.Index) then -- this belief is not selected by others.
		...
		else -- selected, add a teaser
			beliefInst.BeliefLabel:LocalizeAndSetText('[ICON_CheckFail]' .. Locale.ToUpper(row.Name));
			beliefInst.BeliefButton:SetToolTipString(Locale.Lookup('LOC_BELIEF_SELECTED_BY_OTHERS'));
		end
	end
