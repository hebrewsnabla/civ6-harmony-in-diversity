...
values
	...
	("LOC_TOOLTIP_REGIONAL_EFFECT_RANGE_MODIFIER",								"{1_num}-tile Regional Effect: The basic yields of this Building are extend to cities within {1_num} tiles, except those having the same building or those have recieved this bonus from the same building."),
...
values
	...
	("zh_Hans_CN",  "LOC_TOOLTIP_REGIONAL_EFFECT_RANGE_MODIFIER",							"{1_num}环辐射：此建筑的基础产出延伸到{1_num}个单元格内的城市，但不包括有相同建筑或是已经从相同建筑获得此加成的城市。"),
