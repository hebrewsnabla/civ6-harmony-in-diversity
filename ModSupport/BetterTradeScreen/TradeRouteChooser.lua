...
function GetYieldForCity(yieldIndex:number, city:table, originCity:boolean)
    ...
    local multiplier = 1;
    if m_originCity:GetOwner() ~= city:GetOwner() then
        if originCity then
            local pPlayerTrade:table = Players[m_originCity:GetOwner()]:GetTrade();
            multiplier = pPlayerTrade:GetInternationalYieldModifier(yieldIndex);
        else
            local pPlayerTrade:table = Players[city:GetOwner()]:GetTrade();
            multiplier = pPlayerTrade:GetInternationalYieldModifier(yieldIndex);
        end
    end
    if totalValue > 0 and multiplier ~= 1 then
        local totalBeforeMultiplier = totalValue;
        totalValue = totalBeforeMultiplier * multiplier;
        local valueFromMultiplier:number = totalValue - totalBeforeMultiplier;
        local multiplierAsPercent:number = (multiplier * 100) - 100;

        if sourceText ~= "" then
            sourceText = sourceText .. "[NEWLINE]";
        end
        sourceText = sourceText .. Locale.Lookup("LOC_ROUTECHOOSER_YIELD_SOURCE_MULTIPLIERS",
            valueFromMultiplier, yieldInfo.IconString, yieldInfo.Name, multiplierAsPercent);
    end
