...
function OnReadyButton()
	...
	-- MOD VERSION CHECKER
	if(ExposedMembers.MOD_VERSION_CHECKER) then
		if(ExposedMembers.MOD_VERSION_CHECKER.BroadcastVersionDataIfNecessary) then
			ExposedMembers.MOD_VERSION_CHECKER.BroadcastVersionDataIfNecessary()
		end
	end
	--
...
-- MOD VERSION CHECKER BEGINS BELOW --

local function ResetBroadcastStatus()
	ExposedMembers.MOD_VERSION_CHECKER.NeedBroadcast = true
	ExposedMembers.MOD_VERSION_CHECKER.VersionsCompiled = false
	--ExposedMembers.MOD_VERSION_CHECKER.NeedToSearchForMods = true
	ExposedMembers.MOD_VERSION_CHECKER.SearchForMVCCompatibleMods()
end

-- Although redundant, there is technically no harm in running this function multiple times.
-- In a perfect world, it would only need to be run once, right when the lobby opens,
-- so the client can immediately compare other players' broadcasts to its own knowledge.
-- However, due to some bizarre network / mod-loading timing issues, running it in ResetBroadcastStatus doesn't
-- always return accurate results, so to be safe, we also run the function again right before making our broadcast.
local function SearchForMVCCompatibleMods()
	--if(ExposedMembers.MOD_VERSION_CHECKER.NeedToSearchForMods) then
		print("Searching for MVC-compatible mods...")
		local enabledMods = GameConfiguration.GetEnabledMods();
		for _, curMod in ipairs(enabledMods) do
			if(not curMod.Official) then
				-- print(curMod)
				if(pcall(function()
					include( "MVC_" .. curMod.Id )
				end)) then
				else
				end
			end
		end
		--ExposedMembers.MOD_VERSION_CHECKER.NeedToSearchForMods = false
	--end
end

function lookupTitle(mod_id)
	local enabledMods = GameConfiguration.GetEnabledMods();
	for _, curMod in ipairs(enabledMods) do
		if curMod.Id == mod_id then
			return curMod.Title
		end
	end
	return mod_id
...
local function CompileVersionDataFromAllCompatibleMods()
	ExposedMembers.MOD_VERSION_CHECKER.SearchForMVCCompatibleMods()
	local enabledMods = GameConfiguration.GetEnabledMods();
	for _, curMod in ipairs(enabledMods) do
		if(not curMod.Official) then
			if(ExposedMembers.MOD_VERSION_CHECKER.ModVersionData[curMod.Id]) then
			else
				ExposedMembers.MOD_VERSION_CHECKER.ModVersionData[curMod.Id] = 0
			end
			if(ExposedMembers.MOD_VERSION_CHECKER.ModVersionData[curMod.Id]>0) then
				print(curMod.Title .. " has been registered with MVC!")
			else
				print(curMod.Title .. " is not compatible with MVC.")
			end
		end
	end
	ExposedMembers.MOD_VERSION_CHECKER.VersionsCompiled = true;
end

local function BroadcastVersionDataIfNecessary(...)
	if(ExposedMembers.MOD_VERSION_CHECKER.NeedBroadcast) then
		ExposedMembers.MOD_VERSION_CHECKER.CompileVersionDataFromAllCompatibleMods()
		SendChat(ExposedMembers.MOD_VERSION_CHECKER.CompiledModVersionData())
		ExposedMembers.MOD_VERSION_CHECKER.NeedBroadcast = false
	end
	return true
end

local function CompiledModVersionData()
...
local function ParseChatForModVersionCheckerBroadcasts( fromPlayer, toPlayer, text, eTargetType )
	if(Network.GetLocalPlayerID() == 0) then
		ExposedMembers.MOD_VERSION_CHECKER.CompileVersionDataFromAllCompatibleMods()
		if(ExposedMembers.MOD_VERSION_CHECKER.VersionsCompiled) then
			if (fromPlayer ~= Network.GetLocalPlayerID()) then
				if(text:find("Mod Version Checker: ", 1, true) ~= nil) then

					ExposedMembers.MOD_VERSION_CHECKER.SearchForMVCCompatibleMods()
					for m, v in pairs(ExposedMembers.MOD_VERSION_CHECKER.ModVersionData) do
						ExposedMembers.MOD_VERSION_CHECKER.ModVersionComparisonData[m] = false
					end

					local MVCstring = text:gsub("Mod Version Checker: ","")
					local MVCparts = split(MVCstring, ",")
					local allModsOkay = true
					for p, MVCpart in pairs(MVCparts) do
						local MVCsubparts = split(MVCpart, "#")
						ExposedMembers.MOD_VERSION_CHECKER.ModVersionComparisonData[MVCsubparts[1]] = true
						if(ExposedMembers.MOD_VERSION_CHECKER.ModVersionData[MVCsubparts[1]]) then
							if(tonumber(MVCsubparts[2]) == 0) then
								allModsOkay = false
								SendChat("[color:ChatMessage_Whisper]" .. Locale.Lookup(PlayerConfigurations[fromPlayer]:GetPlayerName()) .. " has an unknown version of " .. MVCsubparts[1])
							elseif(tonumber(ExposedMembers.MOD_VERSION_CHECKER.ModVersionData[MVCsubparts[1]]) == tonumber(MVCsubparts[2])) then
								--SendChat("[color:COLOR_GREEN]" .. Locale.Lookup(PlayerConfigurations[fromPlayer]:GetPlayerName()) .. " has the correct version of " .. MVCsubparts[1])
							else
								allModsOkay = false
								SendChat("[color:COLOR_RED]" .. Locale.Lookup(PlayerConfigurations[fromPlayer]:GetPlayerName()) .. " has a mismatched version of " .. MVCsubparts[1])
							end
						end
					end
					for m, v in pairs(ExposedMembers.MOD_VERSION_CHECKER.ModVersionData) do
						if(ExposedMembers.MOD_VERSION_CHECKER.ModVersionComparisonData[m]==false) then
							allModsOkay = false
							SendChat("[color:COLOR_RED]" .. Locale.Lookup(PlayerConfigurations[fromPlayer]:GetPlayerName()) .. " is missing " .. MVCsubparts[1])
						end
					end
					if(allModsOkay) then
						SendChat("[color:COLOR_GREEN]" .. Locale.Lookup(PlayerConfigurations[fromPlayer]:GetPlayerName()) .. "'s mods are all up to date!")
					end
				end
			end
		end
...
function InitModVersionChecker()
	...
	if not ExposedMembers.MOD_VERSION_CHECKER.ModVersionComparisonData then ExposedMembers.MOD_VERSION_CHECKER.ModVersionComparisonData = {} end
	ExposedMembers.MOD_VERSION_CHECKER.NeedBroadcast = true
	ExposedMembers.MOD_VERSION_CHECKER.VersionsCompiled = false
	--ExposedMembers.MOD_VERSION_CHECKER.NeedToSearchForMods = true
	ExposedMembers.MOD_VERSION_CHECKER.ResetBroadcastStatus = ResetBroadcastStatus
	LuaEvents.HostGame_ShowStagingRoom.Add( ResetBroadcastStatus )
	LuaEvents.JoiningRoom_ShowStagingRoom.Add( ResetBroadcastStatus )
	ExposedMembers.MOD_VERSION_CHECKER.SearchForMVCCompatibleMods = SearchForMVCCompatibleMods
	ExposedMembers.MOD_VERSION_CHECKER.CompileVersionDataFromAllCompatibleMods = CompileVersionDataFromAllCompatibleMods
	ExposedMembers.MOD_VERSION_CHECKER.BroadcastVersionDataIfNecessary = BroadcastVersionDataIfNecessary
	--Controls.ReadyButton:RegisterCallback( Mouse.eLClick, BroadcastVersionDataIfNecessary )
	--Controls.ReadyCheck:RegisterCallback( Mouse.eLClick, BroadcastVersionDataIfNecessary )
	ExposedMembers.MOD_VERSION_CHECKER.CompiledModVersionData = CompiledModVersionData
	ExposedMembers.MOD_VERSION_CHECKER.ParseChatForModVersionCheckerBroadcasts = ParseChatForModVersionCheckerBroadcasts
	Events.MultiplayerChat.Add( ParseChatForModVersionCheckerBroadcasts )
	ResetBroadcastStatus()
	print("Mod Version Checker initialized")
end

InitModVersionChecker()