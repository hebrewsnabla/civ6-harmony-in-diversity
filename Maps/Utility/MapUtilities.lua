...
function CanPlaceGoodyAt(improvement, plot)
	...
	-- DeepLogic added: make snow tiles invalid for placing goodyhuts
	...
	for dx = -uniqueRange, uniqueRange, 1 do
		for dy = -uniqueRange, uniqueRange, 1 do
			...
			if (otherPlot and otherPlot:GetImprovementType() == improvementID) then
				if Map.GetPlotDistance(plot:GetIndex(), otherPlot:GetIndex()) <= uniqueRange then
					return false;
				end
			end
		end
	...
	for dx = -3, 3 do
		for dy = -3, 3 do
			...
			if(otherPlot and Map.GetPlotDistance(plot:GetIndex(), otherPlot:GetIndex()) <= 3) then
...
function AddGoodies(iW, iH)
	...
	print("-------------------------------");
	print("Map Generation - Adding Goodies");
	print("Map Generation Fixing -- DeepLogic version");
	...
	for improvement in GameInfo.Improvements() do
		...
		if(improvement.Goody and not (improvement.TilesPerGoody == nil)) then
			...
			local iNeedtoPlace = math.floor(iTiles / improvement.TilesPerGoody);
